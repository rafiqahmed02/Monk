...
ProgramFormFields;
import ProgramFormFields, {
  ProgramFormFieldsProps,
} from "../../../v1/components/MobileViewComponents/ProgramsComponent/ProgramFormFields/ProgramFormFields";
...
import RegistrationOptions from "../../../v1/components/MobileViewComponents/ProgramsComponent/Helper/Components/RegistrationOptions";
import { LocationBasedToday } from "../../../v1/helpers/HelperFunction";
import ReusableDatePicker from "../../../v1/components/MobileViewComponents/ProgramsComponent/helperComponent/ReusableDatePicker";
import NewProgramImageUploader from "../../../v1/components/MobileViewComponents/ProgramsComponent/ImageUploader/NewProgramImageUploader";
...
vi.mock(
  "../../../v1/components/MobileViewComponents/ProgramsComponent/Helper/Components/RegistrationOptions",
...
vi.mock(
  "../../../v1/components/MobileViewComponents/ProgramsComponent/helperComponent/ReusableDatePicker",
  () => ({
    __esModule: true,
    default: () => (
...
// );
vi.mock(
  "../../../v1/components/MobileViewComponents/ProgramsComponent/ImageUploader/NewProgramImageUploader",
  () => ({
    __esModule: true,
...
describe("ProgramFormFields Component", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.clearAllTimers();
  });
  const defaultProps: ProgramFormFieldsProps = {
    isMainAdmin: false,
    isEditMode: false,
    formData: {
      programName: "",
  ...
  //     render(<ProgramFormFields {...defaultProps} />);
  ...
  it("handles category selection", () => {
    ...
    fireEvent.change(categorySelect, {
      target: { name: "category", value: "Community Outreach" },
    });
  ...
  it("renders address input when addressDifferentChecked is true", () => {
    ...
    render(<ProgramFormFields {...props} />);

    console.log(prettyDOM(screen.getByTestId("program-form-fields"), 30000));
  ...
  it("enables Next button when required fields are filled", () => {
    const props = {
      ...defaultProps,
      formData: {
        programName: "Community Gathering",
  ...
  it("shows toast error when required fields are missing on Next button click", async () => {
    ...
    it("prevents default action for keys 'e', 'E', '+', '-'", () => {
      // Render the ProgramFormFields component with default props
      render(<ProgramFormFields {...defaultProps} />);
    ...
    it("submits the form with all values filled and calls setIsPreviewVisible with true on next click", async () => {
      ...
      const { getByText } = render(
        <ProgramFormFields {...defaultProps} formData={filledForm} />
      );
    ...
    it("checks for the cost to have some value if registration is paid", async () => {
      ...
      expect(toast.error).toHaveBeenCalledWith(
        "Cost is required for paid programs."
      );
    });
});
